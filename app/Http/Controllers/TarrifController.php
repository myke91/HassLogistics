...
class TarrifController extends Controller
{
    ...
    public function getTarrifForm()
    {
        ...
        return View::make('tarrif.tarrif.add_tarrif', compact('tarrifs', 'unapprovedInvoices'));
    }
    ...
    public function getTarrifTypeForm()
    {
        ...
        $unapprovedInvoices = \HASSLOGISTICS\InvoiceHeader::where('is_approved', '=', 0)->count();
        return view('tarrif.tarrif_type.add_tarrif_type', compact('tarrifs', 'tarrifTypes', 'unapprovedInvoices'));
    }
    ...
    public function getTarrifParamForm()
    {
        ...
        $tarrifParams = TarrifParams::join('tarrif_type', 'tarrif_type.tarrif_type_id', '=', 'tarrif_params.tarrif_type_id')
            ->get();
        return View::make('tarrif.tarrif_params.add_tarrif_param', compact('tarrifTypes', 'tarrifParams', 'unapprovedInvoices'));
    }
    ...
    public function getTarrifChargeForm(Request $request)
    {
        ...
        $tarrifCharges = TarrifCharge::join('tarrif_params', 'tarrif_params.tarrif_param_id', '=', 'tarrif_charge.tarrif_param_id')
            ->get();
        $unapprovedInvoices = \HASSLOGISTICS\InvoiceHeader::where('is_approved', '=', 0)->count();
        return View::make('tarrif.tarrif_charges.add_tarrif_charge', compact('tarrifCharges', 'tarriParams', 'unapprovedInvoices'));
    }
