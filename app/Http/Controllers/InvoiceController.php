...
class InvoiceController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            Audit::create(['user' => Auth::user()->username, 'activity' => 'Created invoice with invoice number ' . $request->invoice_no, 'act_date' => date('Y-m-d'), 'act_time' => time('H:i:s')]);
    ...
    public function saveAllAndGenerateInvoice(Request $request)
    {
        ...
        Audit::create(['user' => Auth::user()->username, 'activity' => 'Processed and generated invoice for client ' . $invoiceHeader->client . ' on vessel ' . $vessel, 'act_date' => date('Y-m-d'), 'act_time' => time('H:i:s')]);
