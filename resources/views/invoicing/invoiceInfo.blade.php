@extends('layouts.app') @section('content') @include('popups.invoice_details')
<style>
	.del {
		text-align: center;
...
<div class="row">
	<div id="breadcrumb" class="col-xs-12">
		...
		<ol class="breadcrumb pull-left">
			...
			<li>
				<a href="{{route('dashboard')}}">Invoice</a>
			</li>
			<li>
				<a href="{{route('invoiceInfoPage')}}">Invoice History</a>
			</li>
		</ol>
	</div>
</div>
		...
		<div class="box">
			...
			<div class="box-content no-padding">
				<table class="table table-bordered table-striped table-hover table-heading table-datatable paged" id="datatable-3">
					...
					<tbody>
						@foreach($invoices as $key => $value) @if(\Carbon\Carbon::createFromFormat('Y-m-d', $value->due_date)
						< \Carbon\Carbon::now()) <tr style="color:red;">
							@elseif ((\Carbon\Carbon::createFromFormat('Y-m-d', $value->due_date) >= \Carbon\Carbon::now()->subWeeks(3)) && (\Carbon\Carbon::createFromFormat('Y-m-d',
							$value->due_date)
							<=\ Carbon\Carbon::now()->addWeeks(3)))
								<tr style="color:green">
									...
									<tr>
										...
										<td>
											<button value="{{$value->invoice_header_id}}" class="btn btn-link show-invoice-details">
